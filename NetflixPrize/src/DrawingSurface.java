		...
		new Thread() {
			public void run() {
				...
				predictorLoaded = true;
				 ...
				 Movie m = new Movie(recommendedID);
				 
				 int x = Collections.binarySearch(predictor.getMovies(), m);
				 m = predictor.getMovies().get(x);
				 
				 for (int i = 0; i < predictor.getMovies().size(); i++) {
					 if (recommendedID == predictor.getMovies().get(i).getMovieId()) {
						 m = predictor.getMovies().get(i);
					 }
				 }
